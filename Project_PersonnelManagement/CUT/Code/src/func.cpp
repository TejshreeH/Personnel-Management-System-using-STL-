...
void EMPLOYEE::getdata()
{
	...
	while(1)
	{
		...
		try
		{
			if(emp_name.length()<3 || emp_name.length()>20)//checking the length  of the employee name 
			{
				throw emp_name;
			}
		}
	...
	while(1)
	{
		...
		try
		{
			if(contact.length()!=10)//checking the length of the contact entered
			{
				throw contact;
			}
		}
	...
	{
		string salary1;
		cout<<endl<<endl<<"  Enter the Employee's Basic  Salary- ";
		cin>>salary1;
		try
		{
		...
		salary=stod(salary1);
		if(salary<=0)//checking salary should not be zero
		{
			cout<<"\n ----Basic Salary should be greater than Rs. 10,000---\n";
			continue;
		}
		else if(salary<10000 || salary>200000)//checking that salary should be in a range
		{
			...
			else
			{
				...
				for (std::list<PROJECT>::iterator it = pr_list.begin(); it != pr_list.end(); ++it)
				{
					...
					if(it->pr_id==proj_id)//checking the project id matches with the id in the list or not
					{
						...
						pr_no_prid.push_back(proj_id);//pushing the project id in to the vector
						pr_no_hrs.push_back(w_hrs);//pushing number of hours worked in to the vector
